...
출처별 60초 예산과 지수형 대기 상한을 적용합니다. 파이프라인은 canonical URL, 사건
anchor, 제목·본문 유사도를 사용해 다른 언론사의 같은 사건을 대표 원문 한 건으로 합치고,
상태·국가·지역이 다른 사건은 유지합니다. 분야별 발행 기사는 최대 5건입니다. 분류·중복
제거·KST 경계·보안 반례는 `tests/`의 fixture 테스트로 검증합니다.
...
Telegram HTML 메시지는 `💡YY년 M월 D일(요일) 언론 모니터링 결과`와 한 줄을 비운 뒤
`💬오늘의 한마디`로 시작합니다. 지역 또는 군단·사단 기사가 없을 때만
`※ 군단·사단 관련 보도 없음`처럼 표시하며, 다른 빈 분류는 안내하지 않습니다. 기사는
`📍 지역`·`🪖 육군`·`🎖️ 군단·사단`·`🛡️ 국방·안보`·
`🌐 외교·북한`·`📰 칼럼·사설` 순으로 배치하며 분야별 최대 5건을 표시합니다. 각 항목은
