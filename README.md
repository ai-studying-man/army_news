...
Telegram HTML 메시지는 KST 날짜와 날짜별로 고정된 안전한 군 관련 문구를 먼저 표시한 뒤,
각 섹션 앞에 36개 하이픈 구분선을 둡니다. 섹션은 아래 순서로 표시하고 기사 번호는
`1.`부터 시작해 섹션마다 다시 시작합니다. 기사가 없는 섹션은 제목 다음 줄에 `관련 기사
없음`을 표시하며, 같은 사건의 재전재는 대표 기사 한 건만 남깁니다. 각 기사는 제목, 원문에서
확인한 한 문장 실무 참고, 클릭 가능한 링크의 세 논리 줄을 사용하고 항목 사이에는 빈 줄을
둡니다. 제목·참고 문구·URL은 Telegram HTML에 맞게 이스케이프됩니다. RSS 설명이 제목과
출처를 반복하는 경우에는 `세부 내용은 원문 기사에서 확인하세요.`를 대신 표시합니다.
