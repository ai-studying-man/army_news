...
각 기사는 다음 정확한 세 논리 줄을 사용하고 항목 사이에는 빈 줄을 둡니다. Telegram HTML
전송에서는 URL 줄이 이스케이프된 클릭 가능 `<a href="…">…</a>`로 렌더링됩니다. `[사단]`
또는 `[지역]`은 분류 결과에 따라 선택하고, 같은 사건의 재전재는 대표 기사 한 건만 남깁니다.
