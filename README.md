...
각 섹션 앞에 36개 하이픈 구분선을 둡니다. 섹션은 아래 순서로 표시합니다. 기사가 없는
섹션은 제목 다음 줄에 `관련 기사 없음`을 표시하며, 같은 사건의 재전재는 대표 기사 한 건만
남깁니다. 각 기사는 제목, 원문에서
확인한 한 문장 실무 참고, 클릭 가능한 링크 순서로 표시하고 항목 사이에는 빈 줄을 둡니다.
제목·참고 문구·URL은 Telegram HTML에 맞게 이스케이프됩니다. RSS 설명이 없거나 제목과
출처만 반복해 기사 내용에 근거한 요약을 만들 수 없으면 `실무 참고` 줄을 생략합니다.
