...
Telegram HTML 메시지는 다음 세 섹션을 이 순서로 표시합니다. 각 섹션의 기사 번호는 `1.`부터
시작하고 섹션마다 다시 시작합니다. 기사가 없는 섹션은 제목 다음 줄에 `관련 기사 없음`을
표시하며, 같은 사건의 재전재는 대표 기사 한 건만 남깁니다. 각 기사는 세 논리 줄을 사용하고
항목 사이에는 빈 줄을 둡니다. 긴 URL 대신 클릭 가능한 `기사 링크 바로가기` 문구로
렌더링됩니다.
