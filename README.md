...
Telegram HTML 메시지는 `['YY.M.D.(요일), 아침 언론 모니터 결과]`와
`💬오늘의 한마디`로 시작합니다. 기사가 없는 분류는 `※ 군단, 지역 관련 보도 없음`처럼
한 줄로 합쳐 표시합니다. 기사는 1군단·25사단·지역(연천, 파주, 양주)·육군·국방/안보·
외교/북한·칼럼/사설 순으로 배치하며 건수는 제한하지 않습니다. 각 항목은
`[분류] 제목 (언론사)`와 클릭 가능한 링크로 구성합니다. RSS에 제목과 다른 실제 설명이
있을 때만 `-`로 시작하는 50자 이내 첫 문장을 덧붙이고, 제목을 되풀이하는 설명은
표시하지 않습니다.
